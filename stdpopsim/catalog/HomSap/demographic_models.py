...
def _papuans_bougainville_10j19():
    id = "PapuansBougainvilleOutOfAfrica_10J19"
    description = "Out-of-Africa with archaic admixture into Papuans and Bougainvilleans"
    long_description = """
        A eleven population model of out-of-Africa, including two pulses of
        Denisovan admixture into Papuans and Bougainvilleans, and several pulses of Neandertal
        admixture into non-Africans.
        Most parameters are from Jacobs et al. (2019), Table S5 and Figure S5.
        This model is an extension of one from Malaspinas et al. (2016), thus
        some parameters are inherited from there.
    ...
    populations = [
        ...
        _chb_population,
        stdpopsim.Population("Papuan", "Papuans from Indonesia and New Guinea"),
        stdpopsim.Population("Bougainville", "HGDP Bougainville"),
        # archaics
        stdpopsim.Population(
            "DenA", "Altai Denisovan (sampling) lineage", sampling_time=T_DenA
        ),
    ...
    population_configurations = [
        ...
        msprime.PopulationConfiguration(
            initial_size=N_Bougainville, metadata=pop_meta["Bougainville"]
        ),
        msprime.PopulationConfiguration(initial_size=N_DenA, metadata=pop_meta["DenA"]),
        msprime.PopulationConfiguration(initial_size=N_NeaA, metadata=pop_meta["NeaA"]),
        msprime.PopulationConfiguration(initial_size=N_Den1, metadata=pop_meta["Den1"]),
    ...
    m_EU_AS = 3.14e-5
    m_AS_Papuan = 5.72e-5
    # magnitude bigger EAS migration in Bougainville than Papuans
    m_AS_Bougainville = 5.72e-4
    ...
    migration_matrix[pop["CHB"]][pop["Papuan"]] = m_AS_Papuan
    migration_matrix[pop["Papuan"]][pop["CHB"]] = m_AS_Papuan
    # migration_matrix[pop["CHB"]][pop["Bougainville"]] = m_AS_Bougainville
    # migration_matrix[pop["Bougainville"]][pop["CHB"]] = m_AS_Bougainville
    # migration_matrix[pop["Bougainville"]][pop["Papuan"]] = m_Papuan_Bougainville
    # migration_matrix[pop["Papuan"]][pop["Bougainville"]] = m_Papuan_Bougainville
    ...
    demographic_events = [
        ...
        # bottlenecks
        msprime.PopulationParametersChange(
            time=Tb_Bougainville, initial_size=Nb_Bougainville, population_id=pop["Bougainville"]
        ),
        msprime.PopulationParametersChange(
            time=Tb_Eurasia, initial_size=Nb_Eurasia, population_id=pop["CHB"]
        ),
        ...
        msprime.MigrationRateChange(
            time=T_Papuan_Bougainville_split, rate=0, matrix_index=(pop["Bougainville"], pop["Papuan"])
        ),
        msprime.MigrationRateChange(
            time=T_Papuan_Bougainville_split, rate=0, matrix_index=(pop["Bougainville"], pop["CHB"])
        ),
        msprime.MigrationRateChange(
            time=T_EU_AS_split, rate=0, matrix_index=(pop["CHB"], pop["CEU"])
        ),
...
_species.add_demographic_model(_papuans_bougainville_10j19())


def _papuans_bougainville_10j19_simple():
    id = "PapuansBougainvilleOutOfAfrica_10J19_simple"
    description = "Out-of-Africa with archaic admixture into Papuans and Bougainvilleans"
    long_description = """
        A nine population model of out-of-Africa, including two pulses of
        Denisovan admixture into Papuans and Bougainvilleans, and several pulses of Neandertal
        admixture into non-Africans.
        Most parameters are from Jacobs et al. (2019), Table S5 and Figure S5.
        This model is an extension of one from Malaspinas et al. (2016), thus
        some parameters are inherited from there. The ghost and third Denisovan lineage were removed
        """
...
_species.add_demographic_model(_papuans_bougainville_10j19_simple())
...
def _ooa_archaic_plus_american():
    id = "OutOfAfricaArchaicPlusAmericanAdmixture"
    description = "Three population out-of-Africa with archaic admixture plus American admixture"
    long_description = """
        The three population out-of-African model popularized by Gutenkunst et al. (2009)
        and augmented by archaic contributions to both Eurasian and African populations 
        and more recent admixture in the Americas.
        Two archaic populations split early in human history, before the African
        expansion, and contribute to Eurasian populations (putative Neanderthal branch)
        and to the African branch (a deep diverging branch within Africa). Admixture
        is modeled as symmetric migration between the archaic and modern human branches,
        with contribution ending at a given time in the past. 
        Additionally, recent admixture in the Americas is simulated as a single-admixture 
        pulse with 80% contributions from African populations, 19% contributions from European populations, 
